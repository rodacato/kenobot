...
8. [Consciousness Gateway](#consciousness-gateway)
9. [REST API](#rest-api)
10. [Interfaces & Contracts](#interfaces--contracts)
11. [Design Patterns](#design-patterns)
12. [Data Flow](#data-flow)
13. [Limits & Constraints](#limits--constraints)
14. [Composition Root](#composition-root)
15. [Extending KenoBot](#extending-kenobot)
...
> Fast secondary LLM evaluations for keyword expansion, confidence scoring, error classification, and response quality.

The Consciousness Gateway (`src/domain/consciousness/gateway.js`) is a domain port for one-shot LLM calls using expert profiles. It enhances but never blocks the system — all failures return `null` and callers fall back to heuristics.

### Interface

```javascript
const result = await consciousness.evaluate('semantic-analyst', 'expand_keywords', {
  keywords: 'n8n webhook',
  context: 'User debugging auth'
})
// → { expandedKeywords: [...] } or null on any failure
```

### Expert Profiles

Four expert profiles in `templates/experts/`, each with specialized tasks:

| Expert | Tasks |
|--------|-------|
| **semantic-analyst** | `expand_keywords`, `deduplicate_facts`, `extract_patterns`, `evaluate_confidence` |
| **reliability-engineer** | `classify_error`, `extract_lesson` |
| **quality-reviewer** | `evaluate_response`, `detect_hedging` |
| **strategist** | `analyze_sleep_results`, `generate_reflection` |

Each task defines a prompt template with `{variable}` placeholders and an expected JSON output schema. All system prompts enforce JSON-only responses.

### Adapters (3 providers)

| Provider Key | Backend | Requires |
|---|---|---|
| `gemini-cli` (default) | Spawns `gemini` CLI process | `gemini` installed globally |
| `gemini-api` | Google GenAI SDK | `GEMINI_API_KEY` |
| `cerebras-api` | Cerebras Inference API | `CEREBRAS_API_KEY` |

### Two-Tier Configuration

Two separate `ConsciousnessGateway` instances are created in `app.js`:

| Tier | Config Prefix | Default Model | Default Timeout | Use Case |
|---|---|---|---|---|
| **Real-time** | `CONSCIOUSNESS_*` | `gemini-2.0-flash` | 30s | Inline evaluations during chat |
| **Batch** | `CONSOLIDATION_*` | Falls back to `CONSCIOUSNESS_*` | 60s | Sleep cycle / consolidation |

This allows running a fast/cheap model for real-time evaluations and a heavier model for offline consolidation.

### Failure Contract

`evaluate()` returns `null` on: gateway disabled, no adapter, unknown expert/task, adapter error, JSON parse failure. All callers must treat `null` as "use heuristic fallback." Stats tracking (`getStats()`) provides observability into fallback rates.

---

## REST API

> 20 endpoints for programmatic access to conversations, memory, scheduler, sleep cycle, and tasks.

The REST API mounts on the same HTTP server as the webhook channel (requires `HTTP_ENABLED=true` and `API_ENABLED=true`). Requests to `/api/*` are delegated to `APIHandler`.

### Auth & Security

- **Bearer token**: `Authorization: Bearer <API_KEY>` (format: `kb-<64 hex chars>`, auto-generated by `kenobot setup`)
- **Constant-time comparison** via `crypto.timingSafeEqual`
- **Rate limiting**: Sliding window per IP (default: 60 req/min, configurable via `API_RATE_LIMIT`)
- **CORS**: Configurable via `API_CORS_ORIGIN` (default: `*`)
- **Public endpoints**: `GET /api/v1/` and `GET /api/v1/health` (no auth required)

### Endpoints (7 groups)

| Group | Endpoints | Key Operations |
|---|---|---|
| **Discovery** | `GET /` | List all endpoints |
| **Health** | `GET /health`, `GET /stats` | Status, runtime stats |
| **Conversations** | `GET/POST /conversations`, `GET/POST/DELETE /conversations/:id`, `GET /conversations/:id/messages` | CRUD + send messages (synchronous response) |
| **Memory** | `GET /memory`, `GET /memory/recent`, `GET /memory/working/:id`, `GET /memory/patterns` | Read-only access to all memory tiers |
| **Scheduler** | `GET/POST /scheduler`, `DELETE /scheduler/:id` | Manage cron tasks |
| **Sleep Cycle** | `GET /sleep-cycle`, `POST /sleep-cycle/run` | Status and manual trigger (202 Accepted) |
| **Tasks** | `GET /tasks/active`, `GET /tasks/:id/events` | Background task monitoring |
...
Error responses include `error.code`, `error.message`, `error.hint`, and `error.retryable`.

### Key Behaviors

- **One message at a time**: Only one message can be in-flight per conversation. Second message returns `409 Conflict`.
- **Synchronous chat**: `POST .../messages` publishes to the NervousSystem bus as `message:in` and awaits the corresponding `message:out` to return the agent response.
- **Session storage**: API conversations are stored as `api-{id}.jsonl` in the sessions directory.
